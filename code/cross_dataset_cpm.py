...
mri_subs = mri_df['subs']
subs = mri_df.subs.unique()

common = np.isin(sleep_subs, mri_subs)
subj_df['common_ids'] = common


indices = subj_df[subj_df['common_ids'] == False].index
subj_df.drop(indices, inplace=True)




#load in behavioral data
all_behav_data = pd.read_csv(data_dir + '/all_subs.csv', dtype={'Subject': str})

all_behav_data['common_ids'] = common
indices = all_behav_data[all_behav_data['common_ids'] == False].index
all_behav_data.drop(indices, inplace=True)
all_behav_data = all_behav_data.drop(columns=['common_ids'])
...
csv_dir = os.path.join(home,'Desktop', 'hcp', 'rest matrices', 'LR' )
...
#read in matrices
def read_in_matrices(subj_list, file_suffix=None, data_dir=csv_dir, zscore=False):
    """
...
all_fc_data = read_in_matrices(ids, data_dir=csv_dir)
...
uncommon2 = np.setdiff1d(subnums, index)
all_fc_data = all_fc_data.drop(labels=uncommon)
all_behav_data = all_behav_data.drop(labels=uncommon2)
...
data_dir2 = os.path.join(home, 'Desktop', 'abcd_rest')
...
def read_in_matrices2(ids, file_suffix=None, data_dir=data_dir, zscore=False, run=1):
...
fc_data1 = read_in_matrices2(subj_ids1, data_dir=data_dir2, run=1)
fc_data2 = read_in_matrices2(subj_ids2, data_dir=data_dir2, run=2)
fc_data3 = read_in_matrices2(subj_ids3, data_dir=data_dir2, run=3)
fc_data4 = read_in_matrices2(subj_ids4, data_dir=data_dir2, run=4)
...
all_fc_data2 = fc_data.drop(excluded)
final_subs = all_fc_data2.index
...
all_behav_data2 = pd.DataFrame(data)
all_behav_data2.set_index('subject', inplace=True)
...
def cpm_wrapper(all_fc_data, all_behav_data, behav, k=10, **cpm_kwargs):
    ...
    return behav_obs_pred, all_masks, mask_dict
...
all_behav_data2.index.name = None 



cpm_kwargs = {'r_thresh': 0.1, 'corr_type': 'pearson'} 
behav_obs_pred, all_masks, mask_dict = cpm_wrapper(all_fc_data2, all_behav_data2, behav='minutes', **cpm_kwargs)
...
behav_obs_pred2, all_masks2 = cpm_wrapper(all_fc_data, all_behav_data, behav='PSQI_AmtSleep', **cpm_kwargs)
...
def plot_predictions(behav_obs_pred, tail="glm"):
    ...
    x = x['PSQI_AmtSleep observed']
    y = behav_obs_pred.filter(regex=(tail)).astype(float)
    y = y['PSQI_AmtSleep predicted (glm)']
...
#plot
condition = 'HCP Tested on ABCD'
g = plot_predictions(behav_obs_pred2)
