...
behav_obs_pred, all_masks = cpm_wrapper(all_fc_data, all_behav_data, behav='PSQI_AmtSleep', **cpm_kwargs)
#behav_obs_pred2, all_masks2 = cpm_wrapper(wm_fc_data, wm_behav_data, behav='WM_Task_Acc', **cpm_kwargs)
...
def plot_predictions(behav_obs_pred, tail="glm"):
    ...
    g.annotate('r = {0:.2f}'.format(r_val), xy = (0.7, 0.95), xycoords = 'axes fraction')
    g.annotate('p = {0:.2f}'.format(p_val), xy = (0.7, 0.90), xycoords = 'axes fraction')
    g.annotate('n = {}'.format(subs), xy = (0.7, 0.85), xycoords = 'axes fraction')
